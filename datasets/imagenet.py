...
def get_imagenet(data_path, split="train", normalize_data=True, train_with_augmentation=False):
    if split == "train":
    ...
    if not normalize_data:
        transform = transforms.Compose(
            [t for t in transform if not isinstance(t, transforms.Normalize)]
        )
    else:
...
class ImageNetDataset(datasets.ImageFolder):
    ...
    def get_class_ids(self, idx):
        if idx in self.class_ids:
            return self.class_ids[idx]
        else:
            for i, target in enumerate(self.targets):
                if target not in self.class_ids:
                    self.class_ids[target] = []
                self.class_ids[target].append(i)
        return self.class_ids[idx]
...
IMAGENET_CONCEPTS = [c for c in CLASS_NAMES.values()]
...
IN21k_class_labels = np.loadtxt("datasets/imagenet21k_labels.txt", dtype=str, usecols=(0)).tolist()
IN21k_class_labels = [x.lower().replace("_", " ").replace(",", "") for x in IN21k_class_labels]
